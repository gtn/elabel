$( document ).ready(function() {
    ...
    function changeSliderColor() {
    	min = parseInt($( "#min" ).text().substring(4));
    	max = parseInt($( "#max" ).text().substring(4));
    	if($( "#pagevalue" ).val() >= min && $( "#pagevalue" ).val() <= max)
    		$(".exalabel-topicev_grad").css("background-color","#B8D331");
    	else
    		$(".exalabel-topicev_grad").css("background-color","#d33931");
    }
});
